...
# Prefill and decode run on GPUs in the same NVLink-connected host. Keep the
# bulk Mooncake KV transfer off the RoCE HCAs, whose auto-detected multi-NIC
# endpoints can repeatedly reconnect on this machine. Auxiliary data still
# uses TCP. All values remain overridable for hosts without full GPU P2P.
...
export SGLANG_MOONCAKE_SEND_AUX_TCP="${SGLANG_MOONCAKE_SEND_AUX_TCP:-1}"

inner_cmd=""
for variable in \
  PYTHON_BIN SGLANG_JIT_DEEPGEMM_FAST_WARMUP \
  SGLANG_MOONCAKE_CUSTOM_MEM_POOL MC_INTRANODE_NVLINK SGLANG_MOONCAKE_SEND_AUX_TCP \
  ...
  PREFILL_NCCL_PORT DECODE_NCCL_PORT DISAGGREGATION_IB_DEVICE \
